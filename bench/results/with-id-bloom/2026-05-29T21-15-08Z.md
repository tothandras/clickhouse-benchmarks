# Benchmark: with-id-bloom

| Field | Value |
| --- | --- |
| Harness commit | `22f0e1d` |
| ClickHouse | 26.2.19.43 (single-node) |
| Clusters | default |
| Sweep id | `20260529T211111Z` |
| Concurrency | 1 |
| Started | 2026-05-29T21:15:08Z |
| Finished | 2026-05-29T21:16:57Z |

## Ingest

Source `go-seeder`: 10000000 rows in 107.86s (92712 events/sec), batch=10000.

## Queries

| Query | p50 ms | p95 ms | p99 ms | QPS | CPU p50 ms | CPU src | Mem MB |
| --- | ---: | ---: | ---: | ---: | ---: | --- | ---: |
| lookup_by_id | 30.0 | 43.0 | 43.0 | 23.0 | 193.8 | os_cpu | 370.8 |

<details>
<summary><code>lookup_by_id</code></summary>

```sql
SELECT
  om_events.id AS id,
  om_events.type AS type,
  om_events.subject AS subject,
  om_events.source AS source,
  om_events.time AS time,
  om_events.data AS data,
  om_events.ingested_at AS ingested_at,
  om_events.stored_at AS stored_at,
  om_events.store_row_id AS store_row_id
FROM om_events
WHERE om_events.namespace = {namespace:String}
  AND om_events.id = (
    SELECT id FROM om_events
    WHERE namespace = {namespace:String}
    ORDER BY namespace, type, subject, time
    LIMIT 1 OFFSET 100000
  )
```

</details>
