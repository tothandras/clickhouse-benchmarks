# cogs-run-lifecycle Specification

## Purpose

Define the phased execution of a COGS workload cell (prepare, soak, measure, drain, collect, price, report) and its attribution windows, so resource consumption is measured at steady state, merges triggered by measured inserts are captured, and interrupted or unstable runs are flagged rather than silently mispriced.
