# concurrency-sweep Specification

## Purpose

Define the harness's ability to measure every query at more than one concurrency level in a single invocation, recording each level as its own result entry tagged with its concurrency. This turns the "p99 under concurrency roughly halves" class of claim — currently asserted in prose for the bloom-on-`id` index — into a recorded, per-scenario number that can be read as a function of concurrency.
