# benchmark-reports Specification

## Purpose
Render each benchmark run as a human-readable markdown report from the same `Run` record that produces the JSON result file, so a reviewer can read a run in the repo (and on GitHub) without parsing JSON, and the two renderings cannot diverge.
