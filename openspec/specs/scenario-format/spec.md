...
Define the on-disk contract every benchmark scenario must satisfy so the benchmark harness can discover, initialize, and execute scenarios uniformly. A scenario is a self-contained directory describing the ClickHouse schema (`init.sql`), the queries to benchmark (`queries/`), and optionally a seed mechanism, all parameterized so the same scenario can run against any database the driver provides.
